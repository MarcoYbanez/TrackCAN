...
int main(void)
{
	struct spi_buf_set rx_channels[8] = {
		{ .buffers =
			  (struct spi_buf[]){
				  { .buf = (void *)(char[]){ 0b00000000, 0b00000000, 0b00000000 }, .len = 3 },
			  },
		  .count = 1 },
		{ .buffers =
			  (struct spi_buf[]){
	...
	struct spi_buf_set tx_channels[8] = {
		{ .buffers =
			  (struct spi_buf[]){
				  { .buf = (void *)(char[]){ 0b00000110, 0b00000000, 0b00000000 }, .len = 3 },
			  },
		  .count = 1 },
		{ .buffers =
			  (struct spi_buf[]){
				  { .buf = (void *)(char[]){ 0b00000110, 0b01000000, 0b00000000 }, .len = 3 },
			  },
		  .count = 1 },

		{ .buffers =
			  (struct spi_buf[]){
				  { .buf = (void *)(char[]){ 0b00000110, 0b10000000, 0b00000000 }, .len = 3 },
			  },
		  .count = 1 },

		{ .buffers =
			  (struct spi_buf[]){
				  { .buf = (void *)(char[]){ 0b00000110, 0b11000000, 0b00000000 }, .len = 3 },
			  },
		  .count = 1 },

		{ .buffers =
			  (struct spi_buf[]){
				  { .buf = (void *)(char[]){ 0b00000111, 0b00000000, 0b00000000 }, .len = 3 },
			  },
		  .count = 1 },
		{ .buffers =
			  (struct spi_buf[]){
				  { .buf = (void *)(char[]){ 0b00000111, 0b01000000, 0b00000000 }, .len = 3 },
			  },
		  .count = 1 },

		{ .buffers =
			  (struct spi_buf[]){
				  { .buf = (void *)(char[]){ 0b00000111, 0b10000000, 0b00000000 }, .len = 3 },
			  },
		  .count = 1 },

		{ .buffers =
			  (struct spi_buf[]){
				  { .buf = (void *)(char[]){ 0b00000111, 0b11000000, 0b00000000 }, .len = 3 },
			  },
		  .count = 1 },
	};
	...
	while (1)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			spi_transceive_dt(&eng_spi, &tx_channels[i], &rx_channels[i]);
			char *res = (char *)rx_channels[i].buffers[0].buf;

			uint32_t channel_value = 0;
			channel_value |= ((res[1] & 0x0F) << 8) | res[2];
